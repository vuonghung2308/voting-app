    ...
    public class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                var pgsql = OpenDbConnection($"Server={dbHost};Username={dbUsername};Password={dbPassword};Database={dbName};");
                ...
                while (true)
                {
                    ...
                    if (json != null)
                    {
                        ...
                        if (!pgsql.State.Equals(System.Data.ConnectionState.Open))
                        {
                            ...
                            pgsql = OpenDbConnection($"Server={dbHost};Username={dbUsername};Password={dbPassword};Database={dbName};");
                        }
                        else
        ...
        private static ConnectionMultiplexer OpenRedisConnection(string hostname, string password)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    return ConnectionMultiplexer.Connect($"{ipAddress}:6379,password=${password}");
                }
